...
class KittiNuscenesDataset(data.Dataset):
    def __init__(self, kitti_root_path, nuscenes_root_path, mode, num_pc,
                 P_tx_amplitude=10, P_ty_amplitude=0, P_tz_amplitude=10,
                 P_Rx_amplitude=0, P_Ry_amplitude=2.0 * math.pi, P_Rz_amplitude=0,num_kpt=512):
        super(KittiNuscenesDataset, self).__init__()
        self.kitti_root_path = kitti_root_path
        self.nuscenes_root_path = nuscenes_root_path
        self.mode = mode
        self.kitti_dataset = self.make_kitti_dataset(kitti_root_path, mode)
        self.kitti_calibhelper = KittiCalibHelper(kitti_root_path)

        if mode == 'train':
            self.nuscenes_pc_path=os.path.join(nuscenes_root_path,'train','PC')
            self.nuscenes_img_path = os.path.join(nuscenes_root_path,'train', 'img')
            self.nuscenes_K_path = os.path.join(nuscenes_root_path,'train', 'K')
        else:
            self.nuscenes_pc_path = os.path.join(nuscenes_root_path, 'test', 'PC')
            self.nuscenes_img_path = os.path.join(nuscenes_root_path, 'test', 'img')
            self.nuscenes_K_path = os.path.join(nuscenes_root_path, 'test', 'K')
        ...
        self.length = len(self.kitti_dataset) + len(os.listdir(self.nuscenes_pc_path))
...
if __name__ == '__main__':
    dataset = KittiOdometryPairDataset('/zssd/dataset/kitti_odometry/', 'val', 20480)
    data = dataset[4000]
    import pdb
    pdb.set_trace()
