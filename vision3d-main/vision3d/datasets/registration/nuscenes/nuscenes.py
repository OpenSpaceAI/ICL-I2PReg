...
from .nuscenes_t.utils.data_classes import LidarPointCloud
from .nuscenes_t.nuscenes import NuScenes
...
class nuScenesDataset(data.Dataset):
    def __init__(self, root, mode, opt: Options):
    ...
    def __getitem__(self, index):
        ...
        # cv2.imwrite("/ssd/lxj/no_mask_8_21/nuscenes_center_biased/test/img.png", img)
        # cv2.imwrite("/ssd/lxj/no_mask_8_21/nuscenes_center_biased/test/dst.png", dst)
        ...
        if self.opt.is_centered:
            bias = np.mean(pc_np, axis = 1)
            pc_np = pc_np - bias[:, np.newaxis]
            P_np[:3, 3] = P_np[:3, 3] - bias
        ...
        data_dict["image_h"] = self.opt.img_H
        data_dict["image_w"] = self.opt.img_W
        data_dict["intrinsics"] = K.astype(np.float32)
        data_dict["transform"] = P_inv.astype(np.float32)
        data_dict["image"] = img.astype(np.float32)
        data_dict["points"] = pc_np.transpose().astype(np.float32)
        data_dict["feats"] = np.ones(shape=(pc_np.shape[1], 1), dtype=np.float32)
        return data_dict
