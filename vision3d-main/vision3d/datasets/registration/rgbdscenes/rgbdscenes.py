...
class RGBDScenes2D3DHardPairDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # read image
        depth = read_depth_image(osp.join(self.data_dir, metadata["depth_file"])).astype(float)
        image = read_image(osp.join(self.data_dir, metadata["image_file"]), as_gray=self.as_gray)
        
        data_dict["image_path"] = osp.join(self.data_dir, metadata["image_file"])
        ...
        #     fix_idx = np.asarray(range(points.shape[0]))
        #     while points.shape[0] + fix_idx.shape[0] < self.max_points:
        #         fix_idx = np.concatenate((fix_idx, np.asarray(range(points.shape[0]))), axis=0)
        #     random_idx = np.random.choice(points.shape[0], self.max_points - fix_idx.shape[0], replace=False)
